...
[Source](http://www.yinwang.org/blog-cn/2014/04/18/golang "Permalink to 对 Go 语言的综合评价")
...
缺少 generics 是一个问题，然而更严重的问题是 Go 的设计者及其社区对于这类语言特性的盲目排斥。当你提到这些，Go 支持者就会以一种蔑视的态度告诉你："我看不到 generics 有什么用！"这种态度比起语言本身的缺点来说更加有害。在经过了很长一段时间之后 Go 语言的设计者们开始[考虑加入 generics][1]，然后由于 Go 的语法设计偷工减料，再加上由于缺乏 generics 而产生的特例（比如 Go 的 map 的语法设计）已经被大量使用，我觉得要加入 generics 的难度已经非常大。
...
其实如果一个语言有了像 [Typed Racket][2] 和 [PySonar][3] 所支持的 "union type"类型系统，这种多返回值就没有意义了。因为如果有了 union type，你就可以只用一个返回值来表示有效数据或者错误。比如你可以写一个类型叫做 {String, FileNotFound}，用于表示一个值要么是 String，要么是 FileNotFound 错误。如果一个函数有可能返回错误，编译器就强制程序员检查所有可能出现的错误之后才能使用数据，从而可以完全避免以上的各种混淆情况。对 union type 有兴趣的人可以看看 Typed Racket，它拥有我迄今为止见过最强大的类型系统（超越了 Haskell）。
...
举一个相关的例子就是 Go 的 [Sort][4] 函数。每一次需要对某种类型 `T` 的数组排序，比如 `[]string`，你都需要
...
[2]: http://docs.racket-lang.org/ts-guide
[3]: http://yinwang0.wordpress.com/2010/09/12/pysonar
