...
[Source](http://www.yinwang.org/blog-cn/2016/09/18/rust "Permalink to 对 Rust 语言的分析")
...
Rust 的[变量声明][1]跟 Scala 和 Swift 的很像。你用
...
Rust 的文档说它是一种"[大部分基于表达式][2]"的语言，并且给出这样一个例子：
...
Rust 的设计者似乎很推崇"面向表达式"的语言，所以在 Rust 里面你不需要直接写"return"这个语句。比如，这个[例子][3]里面，你可以直接这样写：
...
Rust 的数组可变性标记，跟 Swift 犯了一样的错误。Swift 的问题，我已经在之前的[文章 - Swift 语言的设计错误][4]有详细叙述，所以这里就不多说了。简言之，同一个标记能表示的可变性，要么针对数组指针，要么针对数组元素，应该只能选择其一。而在 Rust 里面，你只有一个地方可以放"mut"进去，所以要么数组指针和元素全部都可变，要么数组指针和元素都不可变。你没有办法制定一个不可变的数组指针，而它指向的数组的元素却是可变的。
...
连 Rust 自己的[文档][5]都说，你可能需要"fight with the borrow checker"。为了通过这些检查，你必须用很怪异的方式来写程序，随着问题复杂度的增加，就要求有更怪异的写法。如果用了 lifetime，很简单一个代码看起来就会是这种样子。真够烦的，我感觉我的眼睛都没法 parse 这段代码了。
...
Rust 的设计者显然受了 [Linear Logic][6] 一类看似很酷的逻辑的启发和熏陶，想用类似的方式奇迹般的解决内存和资源的回收问题。然而研究过一阵子 Linear Logic 之后我发现，这个逻辑自己都没有解决任何问题，只不过给对象的引用方式施加了一些无端的限制，这样使得对象的引用计数是一个固定的值（1）。内存管理当然容易了，可是这样导致有很多程序你没法表达。
...
[1]: https://doc.rust-lang.org/book/variable-bindings.html
[2]: https://doc.rust-lang.org/book/functions.html#expressions-vs-statements
[3]: https://doc.rust-lang.org/book/functions.html#early-returns
[4]: http://www.yinwang.org/blog-cn/2016/06/06/swift
[5]: https://doc.rust-lang.org/book/ownership.html#meta
[6]: https://en.wikipedia.org/wiki/Linear_logic
